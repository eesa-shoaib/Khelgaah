...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, authMiddleware middleware.Middleware) {
	...
	mux.Handle("GET /api/v1/bookings/{id}", authMiddleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		userID, ok := middleware.UserIDFromContext(r.Context())
		if !ok {
			httpx.WriteError(w, http.StatusUnauthorized, "missing user")
			return
		...
		if err != nil {
			...
			httpx.WriteError(w, http.StatusInternalServerError, "failed to get booking")
			return
		}
